...
> **更新（v0.0.2）**：下面描述的「自动对齐响度」按钮和「设备音量设为 100%」按钮**都已经删除**。
> 端点音量现在由每行自己的一个滑杆直接调节，对应的本地化键（`Devices.AutoMatch`、
> `Status.AutoMatch*`）也一并移除，理由是：**能自动的只有第 ② 个因子**，
> 而第 ③ 个（灵敏度）必须靠耳朵——只对齐可测的那一半，反而会让人以为响度已经对齐了。
> 本节保留的分析没有变：三个因子里只有一个是可测的，这正是滑杆必须存在的原因。
