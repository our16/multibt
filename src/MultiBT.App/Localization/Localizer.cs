...
public sealed class Localizer : INotifyPropertyChanged
{
    ...
    private static readonly Dictionary<string, string> Chinese = new(StringComparer.Ordinal)
    {
        ...
        ["Sink.GetCable.Tip"] = "打开 VB-CABLE 的官方下载页（免费）。装好后回到这里，在「捕获出口」里选择它。",
        ["Sink.NoCableWarning"] = "⚠ 未检测到虚拟声卡。当前 Windows 直接渲染到默认设备，而那个设备始终会自己发声——它无法被延迟或调音量。要控制全部设备：点右边的「获取虚拟声卡」装一个免费的 VB-CABLE，然后在上面的「捕获出口」里选择它。",
    ...
    private static readonly Dictionary<string, string> English = new(StringComparer.Ordinal)
    {
        ...
        ["Sink.GetCable.Tip"] = "Opens the official VB-CABLE download page (free). After installing, come back and select it as the capture sink.",
        ["Sink.NoCableWarning"] = "⚠ No virtual audio cable detected. Windows currently renders straight to the default device, so that device always plays natively - it cannot be delayed or volume-controlled. To control every device, install a virtual cable (e.g. the free VB-CABLE) and select it as the capture sink above.",
