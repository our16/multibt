...
public sealed class Localizer : INotifyPropertyChanged
{
    ...
    private static readonly Dictionary<string, string> Chinese = new(StringComparer.Ordinal)
    {
        ...
        ["Sink.Label"] = "音频输入",
        ["Sink.Tip"] = "选择音频来源，可以是虚拟声卡，也可以是某个真实设备。选「自动」时跟随 Windows 默认输出设备。",
        ["Sink.Auto"] = "自动（跟随 Windows 默认输出）",
        ["Sink.Applied"] = "已设置音频输入。",
        ["Sink.AppliedCable"] = "已设置音频输入，并把 Windows 默认输出切到它。",
        ["Sink.Cleared"] = "音频输入已设为自动。",
        ["Sink.SwitchFailed"] = "无法切换 Windows 默认输出设备：{0}",
        ["Sink.GetCable"] = "获取虚拟声卡",
        ["Sink.GetCable.Tip"] = "打开 VB-CABLE 的官方下载页（免费）。装好后在这里选择它即可。虚拟声卡是不会发声的中转：Windows 渲染到它、我们捕获它，因此每个音箱都能单独调延迟和音量。",
        ["Input.RealDeviceNote"] = "当前输入是真实设备「{0}」：它会照常原生播放，所以它自己的延迟和音量不受 MultiBT 控制，也不会收到镜像副本（否则会重复出声）。其他设备正常受控。要控制全部设备，可在「音频输入」里改选一个虚拟声卡。",
    ...
    private static readonly Dictionary<string, string> English = new(StringComparer.Ordinal)
    {
        ...
        ["Sink.Tip"] = "Pick the audio source. This can be a virtual cable or a real device. \"Auto\" follows the Windows default output.",
        ["Sink.Auto"] = "Auto (follow the Windows default output)",
        ["Sink.Applied"] = "Audio input set.",
        ["Sink.AppliedCable"] = "Audio input set to the virtual cable, and the Windows default output was switched to it.",
        ...
        ["Sink.GetCable.Tip"] = "Opens the official VB-CABLE download page (free). Once installed, pick it here. A virtual cable is an inaudible detour: Windows renders into it and we capture it, which is what makes delay and volume controllable on every speaker.",
        ["Input.RealDeviceNote"] = "The input is the real device \"{0}\". It keeps playing natively, so MultiBT does not control its delay or volume, and it does not receive the mirrored copy (that would double its audio). Every other device is controlled normally. To control them all, choose a virtual cable as the audio input.",
